...
const RANKINGS = [
  { rank: 1, player: 'Player A', wins: 45, losses: 5, points: 90 },
  { rank: 2, player: 'Player B', wins: 42, losses: 8, points: 84 },
  { rank: 3, player: 'Player C', wins: 38, losses: 12, points: 76 },
];

export default function ShuttleRankingsPage() {
  return (
          ...
          <div className="bg-gradient-to-r from-teal-500/10 to-transparent border-b border-border px-8 py-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">Player rankings and standings</p>
              </div>
            </div>
          </div>
                      ...
                      {RANKINGS.map((entry) => (
                        <tr key={entry.rank} className="border-b border-border hover:bg-background/50 transition">
                          <td className="py-3 px-4 font-bold">
                            <Badge className="bg-teal-600/20 text-teal-400">#{entry.rank}</Badge>
                          </td>
